...
static int  echeck_abbr __ARGS((int DECL_ASYNC_ARG));
...
static void ins_down __ARGS((int startcol DECL_ASYNC_ARG));
...
# endif
	...
	case Ctrl_H:
	    did_backspace = ins_bs(c, BACKSPACE_CHAR, &inserted_space ASYNC_ARG);
	    auto_format(FALSE, TRUE ASYNC_ARG);
	    break;

	case Ctrl_W:	/* delete word before the cursor */
	    did_backspace = ins_bs(c, BACKSPACE_WORD, &inserted_space ASYNC_ARG);
	    auto_format(FALSE, TRUE ASYNC_ARG);
	    break;
...
# ifdef FEAT_COMPL_FUNC
		...
		goto docomplete;
# endif
	    did_backspace = ins_bs(c, BACKSPACE_LINE, &inserted_space ASYNC_ARG);
	    auto_format(FALSE, TRUE ASYNC_ARG);
	    inserted_space = FALSE;
	    break;
    ...
    DECL_ASYNC_ARG_KR
{
	...
	if (	   mode == BACKSPACE_CHAR
		&& ((p_sta && in_indent)
		    || (get_sts_value() != 0
			&& curwin->w_cursor.col > 0
			&& (*(ml_get_cursor( ASYNC_ARG_ONLY) - 1) == TAB
			    || (*(ml_get_cursor( ASYNC_ARG_ONLY) - 1) == ' '
				&& (!*inserted_space_p
				    || arrow_used))))))
	{
	    int		ts;
    ...
    DECL_ASYNC_ARG_KR
{
	...
	getvcol(curwin, cursor, &want_vcol, NULL, NULL ASYNC_ARG);
